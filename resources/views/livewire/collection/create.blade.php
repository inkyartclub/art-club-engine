<form wire:submit.prevent="submit" class="pt-3">
    ...
    <div class="form-group {{ $errors->has('collection.supply') ? 'invalid' : '' }}">
        <label class="form-label" for="supply">{{ trans('cruds.collection.fields.supply') }}</label>
        <input class="form-control" type="text" name="supply" id="supply" wire:model.defer="collection.supply">
        <div class="validation-message">
            {{ $errors->first('collection.supply') }}
        </div>
        <div class="help-block">
            {{ trans('cruds.collection.fields.supply_helper') }}
        </div>
    </div>
